function [paths,options] = getDataSpecs(PID,dataSet)
...
paths.pilots.expData = [paths.env.data,'pilots',filesep];
paths.jlModel.MainResPath = [paths.env.data,'modeling',filesep,'results',filesep,'main',filesep];
paths.jlModel.PilotResPath = [paths.env.data,'modeling',filesep,'results',filesep,'pilots',filesep];
...
if ~isempty(PID)
    ...
    switch dataSet
        ...
        case 'pilot'
            d = dir([paths.pilots.expData,options.dataSet.acronym,'_*']);
            options.dataSet.nParticipants = size(d,1);
            options.dataSet.PIDs = zeros(options.dataSet.nParticipants,1);
            for i = 1:options.dataSet.nParticipants
                digits = regexp(d(i).name, '[0-9]');
            ...
            for t = 1:options.dataSet.nTasks
                ...
                paths.participant(i).task(t,1).optsFile = [paths.participant(i).behavDir,'SNG_',task,'_',char(string(PID)),'_fmri_optionsFile.mat'];
                paths.participant(i).task(t,1).dataFile = [paths.participant(i).behavDir,'SNG_',task,'_',char(string(PID)),'_fmri_dataFile.mat'];
                paths.participant(i).task(t,2).optsFile = [paths.participant(i).behavDir,'SNG_',task,'_',char(string(PID)),'_behav_optionsFile.mat'];
                paths.participant(i).task(t,2).dataFile = [paths.participant(i).behavDir,'SNG_',task,'_',char(string(PID)),'_behav_dataFile.mat'];
            end
            end
    end
end
